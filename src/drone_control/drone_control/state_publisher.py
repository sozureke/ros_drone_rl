...
class StatePublisher(Node):
    def __init__(self):
        ...
        self._drone_pub = self.create_publisher(DroneState, 'drone/state', 10)
        self._platform_pub = self.create_publisher(PlatformState, 'platform/state', 10)
    ...
    def _on_drone_odom(self, msg: VehicleOdometry):
        # Позиция и скорость
        self._drone_state.x = msg.position[0]
        self._drone_state.y = msg.position[1]
        self._drone_state.z = msg.position[2]
        self._drone_state.vx = msg.velocity[0]
        self._drone_state.vy = msg.velocity[1]
        self._drone_state.vz = msg.velocity[2]

        # Ориентация: кватернион → эйлер
